...
# Basis: Sims equation from Ginsburg, page 285

import numpy as np
import matplotlib.pyplot as plt

def strain_rate_sims(N, R, h1, h2):
    """
    ...
    strain_rate = (np.pi * N / 30) * np.sqrt(R / (r * h1)) * np.log(1 / (1 - r))
...
if __name__=="__main__":
    ...
    # Calculate strain rate
    strain_rate = strain_rate_sims(N, R, h1, h2)
    print(f"Calculated strain rate as per Sims: {strain_rate:.4e} s^-1")

    # generate plot
    r_values = np.arange(0.1, 0.6, 0.1)
    strain_rate_normalized_list = [strain_rate_sims(N, R, h1, h1*(1-r))/(np.pi*N/30 * np.sqrt(R/h1)) for r in r_values]

    # Create the plot
    plt.figure(figsize=(10, 6))
    plt.plot(r_values, strain_rate_normalized_list, label=r'sims', color='blue')
    plt.xlabel("r")
